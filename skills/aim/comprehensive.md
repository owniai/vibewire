...
**严格逐层推进：** 每次只展示当前层的设计，获得用户确认后再进入下一层。不得提前设计或暗示未确认的层级。若存在真实取舍则提出 2-3 个选项并附权衡分析，推荐最优方案及理由；若决策被既有约束唯一确定，直接给出方案并说明依据。
...
**第一步：列举功能单元** — 识别需要实现、修改或调整的原子功能单元（不可再分的最小功能粒度），以列表形式呈现。每个功能单元用一句话描述其目标和范围。

**第二步：划分阶段方案** — 将功能单元按模块归属、功能相关性等组合为阶段，再按依赖关系排序，推荐 1-3 种划分方案。每种方案展示阶段列表，每个阶段以 `Stage {M}-{name} — [一句话描述]` 格式呈现，并说明该方案的特点和适用场景。由用户选择或调整后确认。

**划分原则：**
- 阶段划分忠实反映模块边界和数据流，不通过拆分隐式改变架构决策
- 按**功能**划分，不按**实施步骤**划分。反例：Stage 1-安装依赖、Stage 2-编写代码、Stage 3-集成测试、Stage 4-更新文档
- 每个 Stage 围绕一个功能目标。测试、提交验证、依赖安装、文档更新等是执行流程的内置环节，无需单独列出

### 7. Write Architecture

将步骤 5 和步骤 6 中逐层确认的架构决策和阶段划分整合写入 `.vibewire/{N}-{name}/architecture.md`（按实际确认的层级组装，无则省略）。技术决策注明依据来源（调研结论、实验编号或既有经验）。
