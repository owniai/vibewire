...
- `.vibewire/{N}-{name}/log.md`（如存在）— 前序阶段的执行记录（含漂移）
...
评估结论直接指导 §3 的任务拆分，不在本步骤写入 Drift——漂移记录统一由 §6 在实现完成后写入日志，避免未经验证的预测污染下游。

### 3. Task Breakdown

基于 architecture.md 的 Stage Plan 中本阶段的文件变更清单，结合 §2 的漂移评估结论和 lessons.md 中的经验教训，拆分为自包含的任务。

**拆分原则：**
- **原子性** — 每个 Task 是不可再分的文件变更单元
- **依赖有序** — 被依赖的类型和函数在靠前任务中定义；跨阶段接口签名与 architecture.md 一致（经 §2 确认的偏离除外）
- **功能代码聚焦** — 任务粒度以生产代码变更（功能实现、类型定义、接口实现等）为维度划分；测试编写、验证运行、shadow 更新、提交、日志等是每个任务执行时的标准动作（由 §4-§7 保证），不作为独立任务拆分
...
- 运行全量测试，循环修复直到全部通过：
  ...
  - 若修复后仍失败，且无法解释为何上一次修复应该生效，立即停止 → 回退代码变更（`git checkout -- {当前任务涉及的文件}`），跳至 §8 报告 BLOCKED
...
追加到 `.vibewire/{N}-{name}/lessons.md`（若无则创建并写入 `# Lessons — {N}-{name}` 文件头），记录本阶段实现过程中获得的经验教训。后续 stage 的 implementer 会读取全部累积经验以规避已知陷阱。
...
- 正常完成：返回 `Status: DONE`
- BLOCKED：返回 `Status: BLOCKED` 及原因（直接返回，不写入日志）
