...
**聚焦探索：** 若 `.shadow/` 存在，先读取与本次任务相关的 shadow 文件，快速了解现有模块的声明和类型定义：
- **Shadow 概念**：源文件的声明镜像（类似 .h 头文件），保留 import、类型、接口、类签名、函数签名，省略函数体；行尾 `// L{start}-{end}` 标注指向源文件位置
- **定位规则**：目录结构与源文件严格同构（如 `src/utils/helper.ts` 对应 `.shadow/src/utils/helper.ts`）
- **使用策略**：先读取相关 shadow 文件建立模块全貌，再根据需求定位重点源文件深入阅读。仅读取为评估本次变更所必需的文件，不发散探索无关模块，不逐行阅读大文件（优先读文件头部和关键导出）
...
4. **模块内部架构** — 逐模块展开内部设计
5. **Stage Plan** — 基于已确认的架构，按依赖顺序将模块拆分为渐进式阶段。每个阶段聚焦单一职责、独立可验证，包含 2-5 个实现任务。选定完成端到端路径的阶段作为 Integration Stage，须覆盖所有前序阶段的联合行为。阶段序号 `{M}` 从 1 递增。每个阶段按以下格式描述：
...
- **Integration Stage** — 恰好选定一个完成端到端路径的阶段承载跨 stage 集成验证。Integration Stage 不是独立的测试阶段，而是某个已承载实现职责的阶段，在其实现任务之外额外承担集成验证
