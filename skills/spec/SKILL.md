...
1. **探索项目上下文** — 通过 Explore subagent 深度探索项目的文件、文档、最近提交、相关代码
...
使用 Explore subagent 深度探索当前工作项目，先了解现状再开始规划：

```
subagent_type: "Explore"
description: "探索项目上下文"
prompt: |
  深度探索当前工作项目的上下文，我需要了解项目现状以进行后续规划。请执行以下探索：
  - 读取项目目录结构
  - 读取 README、package.json、配置文件等基础信息
  - 读取 docs/ 目录下的文档
  - 检查最近的 git 提交历史（最近 20 条）
  - 根据项目类型识别技术栈、框架、依赖关系
  - 标注任何与用户任务相关的代码文件及其作用

  请汇总为一个结构化的项目上下文报告，包含：项目类型、技术栈、目录结构概览、关键文件说明、最近开发动态。
```

subagent 完成后，基于探索结果进入下一步。
