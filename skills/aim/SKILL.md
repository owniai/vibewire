...
**聚焦探索：** 若 `.shadow-api/` 存在，先读取与本次任务相关的 shadow-api 文件，快速了解现有模块的接口签名和类型定义。再根据需求定位重点源文件深入阅读。仅读取为评估本次变更所必需的文件，不发散探索无关模块，不逐行阅读大文件（优先读文件头部和关键导出）。
...
- **聚焦探索上下文** — 先 shadow-api 后源文件，仅读取与本次任务相关的文件，充分但不发散
