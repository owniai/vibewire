...
- 非首个里程碑，额外读取前序里程碑的 `.vibewire/{seq}-{name}/milestone-{N}-{name}/tester-log.md` 和 `.vibewire/{seq}-{name}/milestone-{N}-{name}/implementer-log.md`（了解实现过程中的偏差和修复），以及 `.vibewire/{seq}-{name}/shared/index.md` 及相关功能库 API 文档（复用已有公共库）
