...
2. **Drift Assessment** — 评估本阶段是否需要偏离原始规划
...
将拆分结果更新到任务列表，每个任务标注涉及文件及操作（新增/修改/删除）和前置依赖。拆分完成后，将 §4 TDD Loop 更新为每个任务的两个子项：`TDD: {task_name}-Red`（编写测试并确认失败）和 `TDD: {task_name}-Green`（编写实现并确认通过），然后进入 §4 逐任务 TDD 循环。
